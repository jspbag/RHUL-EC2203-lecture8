...
- $\beta_2$ measures the difference in average test scores between a rural district and a non-rural district, holding $STR$ constant
...
- **Dummy variable trap**: $X_{0i}$, $Rural_i$, $Suburban_i$, and $Urban_i$ are **perfectly multicollinear**

  $$Rural_i + Suburban_i + Urban_i = X_{0i} = 1$$
  
  for $i = 1,\ldots,n$
  
- Can't compute OLS estimator w/ perfectly multicollinearity
...
- Different $X_{\ell i}$-values not associated w/ changes in mean $u_i$: $X_{\ell i}$ is **as-if randomly assigned**, $\ell=1,2$
